...
class MainActivity : ComponentActivity() {
    ...
    private suspend fun setLanguage(language: Language) {
        dataStore.updateData {
            it.copy(
                language = language,
                knownLocation = it.knownLocation.mutate { locationList ->
                    /** Actually, The code below should be replaceable but this is just a sample! */
                    locationList.add(Location(32.2, 33.4))
                }
            )
        }
    }
